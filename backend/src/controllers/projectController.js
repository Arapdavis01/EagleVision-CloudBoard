...
exports.getOne = async (req, res) => {
  try {
    ...
    const statusRows = await pool.query(
      `SELECT status_code, response_time_ms, is_up, checked_at FROM uptime_logs WHERE project_id = $1 ORDER BY checked_at DESC LIMIT 1`, [id]
    );
...
exports.update = async (req, res) => {
  ...
  delete updates.id; delete updates.created_at; delete updates.updated_at; delete updates.liveStatus;
  const setClauses = [];
  const values = [];
  let idx = 1;
...
exports.publicStatus = async (req, res) => {
  try {
    ...
    const { rows: projectRows } = await pool.query('SELECT id, name, client_name, live_url, thumbnail_url, status, tech_stack FROM projects WHERE public_token = $1', [token]);
    if (projectRows.length === 0) return res.status(404).json({ error: 'Not found' });
    const project = projectRows[0];
    const { rows: statusRows } = await pool.query(
      `SELECT status_code, response_time_ms, is_up, checked_at FROM uptime_logs WHERE project_id = $1 ORDER BY checked_at DESC LIMIT 1`, [project.id]
    );
